...
func TestGetAlertExpr(t *testing.T) {
	tests := map[string]struct {
		...
		"error_rate": {
			...
			threshold: 0.80,
			expectedExpr: strings.Join([]string{
				"(sum(rate(istio_requests_total{environment=\"test-env\",destination_service_name=~\"test-rev-.*\",request_protocol=\"grpc\",grpc_response_status!=\"0\"}[1m]))/",
				" sum(rate(istio_requests_total{environment=\"test-env\",destination_service_name=~\"test-rev-.*\",request_protocol=\"grpc\"}[1m])) * 100 > 0.800000",
				" and on()",
				" (sum(rate(istio_requests_total{environment=\"test-env\",destination_service_name=~\"test-rev-.*\",request_protocol=\"grpc\"}[1m])) > 0))",
				" or on()",
				" sum(rate(istio_requests_total{environment=\"test-env\",destination_service_name=~\"test-rev-.*\",request_protocol=\"http\",response_code!=\"200\"}[1m]))/",
				" sum(rate(istio_requests_total{environment=\"test-env\",destination_service_name=~\"test-rev-.*\",request_protocol=\"http\"}[1m])) * 100 > 0.800000",
			}, "\n"),
		},
		"cpu_util": {
