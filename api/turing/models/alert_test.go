	...
	tests := map[string]struct {
		...
		"error_rate": {
			...
			expectedExpr: strings.Join([]string{
				...
				" or on()",
				" sum(rate(revision_request_count{environment=\"test-env\",service_name=~\"test-rev-[0-9]*\",response_code_class!=\"2xx\"}[1m]))/",
				" sum(rate(revision_request_count{environment=\"test-env\",service_name=~\"test-rev-[0-9]*\"}[1m])) * 100 > 0.800000",
			}, "\n"),
		},
		"cpu_util": {
