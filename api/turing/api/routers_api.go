...
func (c RoutersController) ListRouters(
	...
	// Append monitoring URL to router
	for _, router := range routers {
		...
		if err != nil {
			return InternalServerError("unable to generate monitoringURL for router", err.Error())
		}
		router.MonitoringURL = monitoringURL
	}
...
func (c RoutersController) CreateRouter(
	...
	rVersion, err := request.BuildRouterVersion(router, c.RouterDefaults, c.AppContext.CryptoService)
	if err == nil {
		// Save router version
