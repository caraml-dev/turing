...
class RouterConfig:
    ...
    def to_open_api(self, use_generic_config: bool = False) -> OpenApiModel:
        """
        This method is currently implemented to handle two different return objects, depending on the boolean argument
        passed. When creating or updating a router, Turing API currently expects RouterConfig objects in the request
        body, though creating a router version requires a RouterVersionConfig object. The only difference between both
        classes is the presence of the environment_name and name fields.

        In the future however, updating routers with RouterConfig objects would be deprecated in favour of using
        RouterVersionConfig objects instead, as the environment_name and name fields are redundant in an update
        operation.

        :param use_generic_config: Boolean flag to indicate if the environment_name and name of this instance should be
            used to return an OpenAPI autogenerated object
        :return: OpenAPIModel representing the router configuration
        """
        kwargs = {}
