...
@responses.activate
def test_create_version(turing_api, active_project, generic_router_config, generic_router_version, use_google_oauth):
    turing.set_url(turing_api, use_google_oauth)
    turing.set_project(active_project.name)
    ...
    actual_response = base_router.create_version(generic_router_config)
    ...
    assert actual_response.experiment_engine.to_open_api() == generic_router_version.experiment_engine
    assert actual_response.resource_request.to_open_api() == generic_router_version.resource_request
    assert actual_response.timeout == generic_router_version.timeout

    assert actual_response.log_config.to_open_api().result_logger_type == generic_router_version.log_config.result_logger_type

    assert actual_response.enricher.image == generic_router_version.enricher.image
    assert actual_response.enricher.resource_request.to_open_api() == generic_router_version.enricher.resource_request
