import pytest
from turing.router.config.route import MissingServiceMethodException, Route, InvalidUrlException
...
@pytest.mark.parametrize(
    ...
    [
        pytest.param(
            "model-a", "http://predict_this.io/model-a", "100ms", None, None, "generic_route"
        ),
        pytest.param(
            "model-a", "http://predict_this.io/model-a", "100ms", Protocol.HTTP, None, "generic_route"
        ),
        pytest.param(
            "model-a-grpc", "grpc_host:80", "100ms", Protocol.UPI, "package/method", "generic_route_grpc"
        ),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        pytest.param(
            "route_test_1", "http//test_this_route.com/", 100, Protocol.HTTP, InvalidUrlException
        ),
        pytest.param(
            "route_test_2", "http//test_this_route.com/", 100, Protocol.UPI, MissingServiceMethodException
        )
    ],
)
